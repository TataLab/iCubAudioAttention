...
spacing=.15;%space between Mics
speedSound=340.5;% m/s
numElements=2;%Number of Mics
nPastFrames=1; %how many frames to go back to compute background means

A=audiorecorder(SamplingFre,24,2);
...
while(~done)
    ...
    %display(['processing frame ' num2str(frameNumber)]);
    ...
    [frameL,frameR]=GetCurrentAudioFrame_builtin(frameDuration_seconds,A);
    %[frameL,frameR,yy]=GetCurrentAudioFrame_prerecorded(yy,frameDuration_samples);
    ...
    deltaAmp_frameR=(amp_frameR-mean(pastAmpR,1))./mean(pastAmpR,1); %try a scaled delta amplitude
    ...
    [spectralPeakValuesL,~]=findpeaks(deltaAmp_frameL);
    [spectralPeakValuesR,~]=findpeaks(deltaAmp_frameR);
   
    
    %handle the not-so-impossible case of zero peaks
    if(isempty(spectralPeakValuesL))
        [spectralPeakValuesL,~]=max(deltaAmp_frameL); %just use the single biggest peak
    end
    
    if(isempty(spectralPeakValuesR))
        [spectralPeakValuesR,~]=max(deltaAmp_frameR); %just use the single biggest peak
    end
    ...
    %normalize the audioSalience [0,1]
    audioSalienceN=(audioSalience-saliencyMin)/(saliencyMax-saliencyMin);
    audioSalienceN(audioSalienceN<0)=0; %arbitrarily clamp them and manually adjust the range
...
%     display(audioSalienceN);
   ...
   %plot
    [xpeak,ypeak]=pol2cart(-90*pi/180,1);
    compass(xpeak,ypeak,'w')
    hold on;
    [xpeak,ypeak]=pol2cart(azimuth*pi/180,audioSalienceN);
...
% %         
% %         display(Vector_of_talkers);
% %         
% %         if new_angle_approval==1
% %             azimuth=Vector_of_talkers(1,1);%the angle that send to robot
% %             
% %         else %rejected? send the preceding angle
% %             azimuth=past_Output_angle;
% %         end
% %         past_Output_angle=azimuth;%update the angle
% %         
% %             %%%%%%%
% %     %plot
% %      [xpeak,ypeak]=pol2cart(-90*pi/180,1);
