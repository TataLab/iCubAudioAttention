...
pastAmp=ones(4,numchans).*.0001; %we have to seed this with some arbitrarily small numbers
pastDeltaAmp=zeros(1,numchans);
...
% n1=20*(rand(1,N)-.5); % Create vector of iterative localized sound with a standard deviation of var.
var=5;% variance of the data
n1=var*(randn(1,N));

n=zeros(2,N);
n(2,:)=n1;
%x=zeros(2,N); % x will be the initialized variable for where the robot hears the sound.
...
for k=2:length(x); %floor(length(yy(:,1))/Steps)
    %         pause
    ...
    if Energy>Threshold_Enregy % to check the sailence (quiet state)
        ...
        DOA=BeamformerFunction(GF_frameL,GF_frameR,SamplingFre);
