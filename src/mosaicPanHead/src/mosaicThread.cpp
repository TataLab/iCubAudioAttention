...
void mosaicThread::run() {
    ...
    if(imagePortInLeft.getInputCount()) {    
        ...
        if (inputImageLeft != NULL ) {
            yDebug("found RGB image \n"); //use either yInfo or yError
            if(!resized) {
    ...
    //----------------------------------------------------------------
    if(imageMonoInLeft.getInputCount()) {  
...
void mosaicThread::makeMosaic(ImageOf<yarp::sig::PixelRgb>* inputImageLeft, ImageOf<PixelRgb>* inputImageRight) {
    ...
    */
    ...
    yDebug("azimuth angle %f", encoders[0]);
    x[0] = encoders[0];  //azimuth
    x[1] = 0;            //elevation
    x[2] = 0;            // 
    ...
    printf(" shiftx %f  fxl %f pixelSize %d \n",shiftx,fxl, pixelSizeLeft);        
    ...
    lineOutTemp = outTemp + mosaicY * (rowSize + mPad) + pixelSizeLeft * mosaicX;
    outTemp = lineOutTemp;
    printf("pixelSize for the left image %d %d %d with padding %d \n", pixelSizeLeft, iH , iW, mPad);
    for(i = 0 ; i < iH ; ++i) {
        for(j = 0 ; j < iW ; ++j) {
            
            if((*inpTemp < 5) && (*(inpTemp+1) < 5) && (*(inpTemp+2) < 5)) { 
                // red 
                inpTemp++; outTemp++;
