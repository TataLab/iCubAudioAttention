...
protected:
    ...
    BufferedPort<Bottle> inportAzimuth;
    ...
    void getSensorData()
    {
        ...
        else if (Bottle *azimuthNew=inportAzimuth.read(false))
        {
        ...
        {    
            ...
            if (state==STATE_IDLE)
            {
                ...
                fprintf(stdout,"--- Got target => REACHING for %s \n", targetPos.toString().c_str());
...
public:
    ...
    bool threadInit()
    {
      printf("starting the managerThread ... \n");
        // general part
        ...
        // open ports
        inportAzimuth.open((name+"/azimuth:i").c_str());
